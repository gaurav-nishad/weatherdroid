 ...
 *
 */
public class DetailsParser implements Parser {
	...
	private  void parseHourCondition(String str,
			String[] units,
			ArrayList<HashMap<String, String>> todayData) {

		// 不包含天气信息则返回
		if (!str.contains("mpdHMHrObj")
				&& !str.contains("hour["))
			return;
		
		HashMap<String, String> hourData = new HashMap<String, String>();

		String time = str.substring(str.indexOf("Date(") + 4,
				str.lastIndexOf(")"));
		
		hourData.put(WeatherKey.TIME, UnitConvert.convertTime("(yyyy,MM,dd,HH,mm,ss)", time));

		String all_str = str.substring(str.indexOf("),") + 2,
				str.lastIndexOf(")"));
	...
	public void parse() {
		...
		BufferedReader reader=new BufferedReader(new InputStreamReader(source.getByteStream()));
		...
		// 取得单位制式
		units= data[line].substring(data[line].indexOf("(") + 1,data[line].indexOf(")")).split(", ");
		...
		wData.bDetailsParsed=true;
	}
	
	
	public DetailsParser(InputSource source,WeatherData wData) {
		this.source=source;
		this.wData=wData;
	}
	...
	public static void main(String[] args) {
		...
		try {
			//source = DataSourceManager.getInputSource("http://www.weather.com/weather/mpdwcr/dailydetails?locid=CHXX0141");
			source = DataSourceManager.getInputSource(file);
			new DetailsParser(source, wd).parse();
		} catch (WeatherException ex) {
			// TODO 自动生成 catch 块
			ex.printStackTrace();
		}
		
		if(wd.hasDetailsParsed()) {
